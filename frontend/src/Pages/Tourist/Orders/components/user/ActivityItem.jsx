...
const ActivityItem = ({ order }) => {
  ...
  const handleNavigation = () => {
    if (!order.isCancelled) {
      navigate(`/Tourist/details/${order._id}`, { state: order });
    }
  };
   const styles = {
    orderItemDetails: () => ({
...
>
  ...
  >
    ...
    <div className="order-info-item" style={{ textAlign: 'center' }}>
      ...
      >
        {new Date(order.ActivityId.start_date).toLocaleDateString()}
      </span>
    </div>
    ...
    <div className="order-info-item" style={{ textAlign: 'center' ,marginLeft:"100px"}}>
      ...
      >
        {new Date(order.ActivityId.end_date).toLocaleDateString()}
      </span>
    </div>
  ...
  <div
    ...
    style={{
      backgroundColor: !order.isCancelled
        ? 'linear-gradient(90deg, #ff0000 50%, #00aaff 50%)' // Brighter and more contrasting gradient
        : '#e0e0e0', // Light gray for disabled
      borderColor: !order.isCancelled ? '#ff0000' : '#b0b0b0', // Distinct border color
