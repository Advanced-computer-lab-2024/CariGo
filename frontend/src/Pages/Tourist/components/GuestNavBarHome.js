...
function TouristNB() {
  ...
  useEffect(() => {
    const fetchUnReadNotifications = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:4000/cariGo/notifications/unread",
          {
    ...
    const fetchNotifications = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:4000/cariGo/notifications/",
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        const readNotifications = allNotifications.filter(
          (notification) =>
            !unReadNotifications.some(
              (unread) => unread._id === notification._id
            ) // Adjust "id" to your unique key
        );
  ...
  const markAsRead = async (id) => {
    try {
      ...
      const updatedSortedNotifications = sortedNotifications.map(
        (notification) =>
          notification._id === id
