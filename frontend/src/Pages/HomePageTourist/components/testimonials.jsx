...
export default function Testimonials() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setItineraries(fetchedItineraries.slice(0, 9)); // Only take the first 9 results
        ...
        setActivities(fetchedActivities.slice(0, 9)); // Only take the first 9 results
      } catch (error) {
  ...
  useEffect(() => {
    if (category === 4) {
      // Most Recent
      const sortedItems = [
        ...itineraries.map((it) => ({ ...it, type: "itinerary" })),
        ...activities.map((act) => ({ ...act, type: "activity" })),
      ].sort(
        (a, b) =>
      ...
      if (sortedItems.length > 0) {
        ...
        if (mostRecent.type === "itinerary") {
          setSortedItineraries([mostRecent]);
          setSortedActivities([]);
        } else {
          setSortedItineraries([]);
          setSortedActivities([mostRecent]);
        }
      }
    } else if (category === 5) {
      // Cheapest
      ...
      setSortedItineraries(
        allItems.filter((item) => item.type === "itinerary").slice(0, 1)
      );
      setSortedActivities(
        allItems.filter((item) => item.type === "activity").slice(0, 1)
      );
    } else {
      setSortedItineraries([]);
      setSortedActivities([]);
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6">
      <div className="border-t py-12 [border-image:linear-gradient(to_right,transparent,theme(colors.slate.400/.25),transparent)1] md:py-20">
        ...
        <div>
          ...
          >
            {[...itineraries, ...activities].map((item, index) => (
              <div
                key={index}
                className={`group ${
                  category !== 1 &&
                  !(
                    (category === 2 && item.date) ||
                    (category === 3 && item.start_date) ||
                    (category === 4 && sortedItineraries.includes(item)) ||
                    (category === 4 && sortedActivities.includes(item)) ||
                    (category === 5 && sortedItineraries.includes(item)) ||
                    (category === 5 && sortedActivities.includes(item))
                  )
                    ? "opacity-40"
                    : ""
                }`}
              >
                <Testimonial
                  item={item}
                  category={category}
                  type={item.date ? "activity" : "itinerary"}
...
function Testimonial({
  ...
  const isDimmed =
    category !== 1 &&
    ((category === 2 && type !== "activity") ||
      (category === 3 && type !== "itinerary") ||
      (category === 4 &&
        ((type === "itinerary" &&
          sortedItineraries.length > 0 &&
          sortedItineraries[0].id !== item.id) ||
          (type === "activity" &&
            sortedActivities.length > 0 &&
            sortedActivities[0].id !== item.id))) ||
      (category === 5 &&
        ((type === "itinerary" &&
          sortedItineraries.length > 0 &&
          sortedItineraries[0].id !== item.id) ||
  ...
  return (
    <article
      className={`relative rounded-2xl p-5 backdrop-blur-sm transition-opacity before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border before:border-transparent ${
        isDimmed
          ? "opacity-40"
          : "bg-gradient-to-br from-[#037bba] via-[#01324c] to-[#037bba]"
      } before:[background:linear-gradient(to_right,theme(colors.gray.800),theme(colors.gray.700),theme(colors.gray.800))_border-box] before:[mask-composite:exclude_!important] before:[mask:linear-gradient(white_0_0)_padding-box,_linear-gradient(white_0_0)]`}
    >
      <div className="flex flex-col gap-4">
        <h3 className="text-xl font-semibold text-white">
          {item.title || item.name}
        </h3>
        <p className="text-gray-300">{item.description || item.content}</p>
        ...
        <div className="text-sm font-medium text-gray-200">
          <span>Author: {item.author?.name || item.name}</span>
        </div>
      </div>
    </article>
  );
