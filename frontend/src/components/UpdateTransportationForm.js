...
export default function UpdateTransportationForm() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/cariGo/transportation/updateTransportation/${id}`, {
        method: 'PATCH',
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', width: '560px', padding: '10px', color: '#ff4d4d', borderRadius: '15px', boxShadow: '0px 1px 3px rgba(0, 0, 0, 0.1)', gap: '5px', margin: '20px', border: '2px solid #126782', paddingLeft: '30px' }}>
      <Box sx={{ marginLeft: '50px' }}>
        <h3 style={{ color: '#ff4d4d' }}>UPDATE A NEW TRANSPORTATION</h3>
        ...
        {/* Submit Button */}
        <Button onClick={handleCreate}>UPDATE TRANSPORTATION</Button>
      </Box>
    </Box>
  );
}
